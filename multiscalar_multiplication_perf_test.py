...
from profiler import PrecompiledKernelWrapper, Profiler, collect_logs
...
class FusionMSMPerformanceTest(parameterized.TestCase):
  ...
  def _create_fusion_msm_wrapper(self, kernel_name, ctx, tiled_slices):
    """Profile ``_multiscalar_multiply_fused`` — the individual jit-able
    kernel. ``ctx.multiscalar_multiply`` is a Python dispatcher that slices
    inputs, places them on shardings, and hashes into ``compiled_kernels``;
    it isn't itself a kernel. We reproduce its setup once here, then hand
    the already-compiled executable + concrete inputs to the profiler.
    """
    ...
    if ctx.use_sharding:
      regular_tiled_slices = regular_tiled_slices.to_device(ctx.fused_reg_slices_sharding)
      last_tiled_slices = last_tiled_slices.to_device(ctx.fused_last_slices_sharding)
    ...
    compiled_fn = ctx.compiled_kernels[kernel_hash]["multiscalar_multiply_fused"]
    # compiled_fn = None
