...
class KernelWrapper(KernelWrapperBase):
  ...
  def _compile(self):
    ...
    if self.enable_sharding and self.mesh:
      ...
      self.jit_lower = jax.jit(jax.named_call(compiled_kernel_function, name=self.kernel_name)).lower(
          *jax_input_structs
      )
    ...
    if "NVIDIA" in device_kind:
      for e in trace_events:
        if "args" in e and "tf_op" in e["args"]:
          ...
          if "compiled_kernel_function" in e["args"].get("hlo_module", "") or "compiled_kernel_function" in e[
              "args"
          ].get("long_name", ""):
            merged_event = False
