...
class Distributor {
    public:
        Distributor(uint32_t window_num, uint32_t regular_bucket_num, uint32_t special_bucket_num, uint32_t msm_length)
            : window_num_(window_num), regular_bucket_num_(regular_bucket_num), special_bucket_num_(special_bucket_num), msm_length_(msm_length) {
        ...
        void set_output_buffers(uint32_t* regular_buckets, uint32_t* special_buckets) {
            ...
            uint32_t regular_bucket_capacity_in_bytes = fixed_regular_padding_size_ * sizeof(point_t);
            uint32_t special_bucket_capacity_in_bytes = fixed_special_padding_size_ * sizeof(point_t);
            uint32_t regular_window_capacity_in_bytes = regular_bucket_num_ * regular_bucket_capacity_in_bytes;
            uint32_t special_window_capacity_in_bytes = special_bucket_num_ * special_bucket_capacity_in_bytes;
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                for (uint32_t j = 0; j < this->bucket_pointers_[i].size(); ++j) {
                    ...
                    this->bucket_pointers_[i][j] =
                        reinterpret_cast<point_t*>(reinterpret_cast<uint8_t*>(regular_buckets_) +
            ...
            for (uint32_t j = 0; j < this->bucket_pointers_[window_num_ - 1].size(); ++j) {
                this->bucket_sizes_[window_num_ - 1][j] = 0;
                this->bucket_pointers_[window_num_ - 1][j] =
                    reinterpret_cast<point_t*>(reinterpret_cast<uint8_t*>(special_buckets_) +
        ...
        void distribute_a_window(int32_t* slices, point_t* points, uint32_t bucket_num,
                                std::vector<std::vector<point_t>>& buckets, uint32_t& max_size) {
            // Implement the distribution logic for a single window here
            buckets.resize(bucket_num);
            uint32_t reserve_num = static_cast<uint32_t>(msm_length_ * reserve_ratio_ / bucket_num);
        ...
        void distribute_a_window_to_buffer(int32_t* slices, point_t* points, uint32_t tile_length, uint32_t max_bucket_size,
                                            std::vector<point_t*>& buckets, std::vector<uint32_t>& bucket_sizes, uint32_t& overflow) {
        ...
        void distribute_a_window_to_buffer_with_sign(int32_t* slices, point_t* points, point_t* neg_points, uint32_t tile_length, uint32_t max_bucket_size,
                                            std::vector<point_t*>& buckets, std::vector<uint32_t>& bucket_sizes, uint32_t& overflow) {
             for (uint32_t i = 0; i < tile_length; ++i) {
                if(slices[i] == 0) {
                    continue;
        ...
        void pad_a_window(point_t* zero, uint32_t target_size, std::vector<std::vector<point_t>>& buckets) {
        ...
        void pad_a_window_to_buffer(point_t* zero, uint32_t target_size, std::vector<point_t*>& buckets,
        ...
        void distribute() {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                distribute_a_window(slices, points_list_, regular_bucket_num_, windows_[i], max_regular_bucket_size);
            }
            uint32_t max_special_bucket_size = 0;
            auto end2 = std::chrono::high_resolution_clock::now();
            distribute_a_window(slices_list_pointers_[window_num_ - 1],
                                points_list_, special_bucket_num_,
                                windows_[window_num_ - 1],
                                max_special_bucket_size);
            end = std::chrono::high_resolution_clock::now();

            duration_us = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
            auto duration2_us = std::chrono::duration_cast<std::chrono::microseconds>(end2 - start).count();
            ...
            for (uint32_t i = 0; i < window_num_; ++i) {
                uint32_t target_size = (i == window_num_ - 1) ? max_special_bucket_size : max_regular_bucket_size;
        ...
        void distribute_parallel_v1() {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                int32_t* slices = slices_list_pointers_[i];
                distribute_a_window(slices, points_list_, regular_bucket_num_, windows_[i], max_regular_bucket_sizes[i]);
            }
            auto end2 = std::chrono::high_resolution_clock::now();
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
        ...
        void distribute_a_window_to_buffer_zero_indexed(int32_t* slices, point_t* points, uint32_t tile_length, uint32_t max_bucket_size,
                                            std::vector<point_t*>& buckets, std::vector<uint32_t>& bucket_sizes, uint32_t& overflow) {
             for (uint32_t i = 0; i < tile_length; ++i) {
        ...
        void distribute_to_buffer_zero_indexed_parallel() {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                distribute_a_window_to_buffer_zero_indexed(slices, points, msm_length_, max_bucket_size,
                                            buckets, bucket_sizes, bucket_overflow[i]);
            }
            #ifdef PROFILE
                auto end2 = std::chrono::high_resolution_clock::now();
            #endif

            distribute_a_window_to_buffer_zero_indexed(slices_list_pointers_[window_num_ - 1],
                                          points_list_, msm_length_,
                                          this->fixed_special_padding_size_,
                                          bucket_pointers_[window_num_ - 1],
                                          bucket_sizes_[window_num_ - 1],
            ...
            #pragma omp parallel for
            for (uint32_t i = 0; i < window_num_; ++i) {
                uint32_t target_size = (i == window_num_ - 1) ? this->fixed_special_padding_size_ : this->fixed_regular_padding_size_;
                pad_a_window_to_buffer(zero_, target_size, bucket_pointers_[i], bucket_sizes_[i]);
            }
            #ifdef PROFILE
                end = std::chrono::high_resolution_clock::now();
                duration_us = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
        ...
        void distribute_to_buffer_parallel() {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                std::vector<uint32_t>& bucket_sizes = bucket_sizes_[i];
                uint32_t max_bucket_size = this->fixed_regular_padding_size_;
                distribute_a_window_to_buffer(slices, points, msm_length_, max_bucket_size,
                                            buckets, bucket_sizes, bucket_overflow[i]);
            }
            #ifdef PROFILE
                auto end2 = std::chrono::high_resolution_clock::now();
            ...
            distribute_a_window_to_buffer(slices_list_pointers_[window_num_ - 1],
                                          points_list_, msm_length_,
                                          this->fixed_special_padding_size_,
                                          bucket_pointers_[window_num_ - 1],
                                          bucket_sizes_[window_num_ - 1],
        ...
        void distribute_to_buffer_signed_parallel() {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                std::vector<uint32_t>& bucket_sizes = bucket_sizes_[i];
                uint32_t max_bucket_size = this->fixed_regular_padding_size_;
                distribute_a_window_to_buffer_with_sign(slices, points,neg_points, msm_length_, max_bucket_size,
                                            buckets, bucket_sizes, bucket_overflow[i]);
            }
            #ifdef PROFILE
                auto end2 = std::chrono::high_resolution_clock::now();
            ...
            distribute_a_window_to_buffer_with_sign (slices_list_pointers_[window_num_ - 1],
                                          points_list_, neg_points_list_, msm_length_,
                                          this->fixed_special_padding_size_,
                                          bucket_pointers_[window_num_ - 1],
                                          bucket_sizes_[window_num_ - 1],
                                          bucket_overflow[window_num_ - 1]);
        ...
        void distribute_to_buffer_parallel_v2() {
            ...
            for(uint32_t t = 0; t < tile_num_; ++t) {
                ...
                for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                    ...
                    std::vector<uint32_t>& bucket_sizes = bucket_sizes_[i];
                    uint32_t max_bucket_size = this->fixed_regular_padding_size_;
                    distribute_a_window_to_buffer(slices, points, tile_length_, max_bucket_size,
                                                buckets, bucket_sizes, bucket_overflow[i]);
                }
            }
            #ifdef PROFILE
            ...
            distribute_a_window_to_buffer(slices_list_pointers_[window_num_ - 1],
                                          points_list_, msm_length_,
                                          this->real_special_padding_size_,
                                          bucket_pointers_[window_num_ - 1],
                                          bucket_sizes_[window_num_ - 1],
                                          bucket_overflow[window_num_ - 1]);
        ...
        void get_merged_regular_buckets(uint32_t* regular_buckets) {
            ...
            for (uint32_t i = 0; i < window_num_ - 1; ++i) {
                ...
                for (uint32_t j = 0; j < bucket_size; ++j) {
                    std::memcpy(regular_buckets + offset, windows_[i][j].data(), windows_[i][j].size() * sizeof(point_t));
        ...
        void get_merged_special_buckets(uint32_t* special_buckets) {
            ...
            for (uint32_t j = 0; j < bucket_size; ++j) {
                std::memcpy(special_buckets + offset, windows_[window_idx][j].data(), windows_[window_idx][j].size() * sizeof(point_t));
...
distributor_params_t init_distributor_param(uint32_t slice_length,
    ...
    uint32_t log_special_duplication_ratio = static_cast<uint32_t>(
        std::ceil(std::log2(static_cast<double>(regular_bucket_num) / special_bucket_num))
    );
    uint32_t special_duplication_ratio = 1U << log_special_duplication_ratio;
    uint32_t bucket_num_duplication = special_bucket_num * special_duplication_ratio;

    // Output buffer parameter initialization
    double expected_regular_bucket_size = static_cast<double>(msm_length) / (regular_bucket_num + 1);
    double expected_special_bucket_size = static_cast<double>(msm_length) / bucket_num_duplication;
    uint32_t regular_bucket_size = static_cast<uint32_t>(expected_regular_bucket_size * buf_extend_ratio);
    uint32_t special_bucket_size = static_cast<uint32_t>(expected_special_bucket_size * buf_extend_ratio);
    ...
    params.fixed_special_padding_size = special_bucket_size * special_duplication_ratio;
    params.zero = new uint32_t[COORDINATE_NUM * CHUNK_NUM](); // Initialize to zero
    params.slices_list = new int32_t[window_num * msm_length];
    params.points_list = new uint32_t[msm_length * COORDINATE_NUM * CHUNK_NUM];
    params.neg_points_list = new uint32_t[msm_length * COORDINATE_NUM * CHUNK_NUM];
...
int main_old() {
    ...
    std::cout << "Signed Bucket: " << (signed_bucket ? "true" : "false") << std::endl;
     

    for (uint32_t i = 0; i < repeat; ++i) {
        std::cout << "Run " << i + 1 << " of " << repeat << std::endl;
        distributor_params_t params = init_distributor_param(slice_length, msm_length, buf_extend_ratio, signed_bucket);
       
        auto start_time = std::chrono::high_resolution_clock::now();

        Distributor* distributor = new Distributor(
            params.window_num, 
            params.regular_bucket_num, 
            params.special_bucket_num, 
            params.msm_length
        );


        distributor->set_fixed_padding_size(params.fixed_regular_padding_size, params.fixed_special_padding_size);
        ...
        distributor->set_output_buffers(params.regular_buckets, params.special_buckets);
    ...
    std::cout << "Average distribution time: " << (duration_us / repeat) << " us" << std::endl;
...
double run_performance_test(uint32_t slice_length, uint32_t msm_length, double buf_extend_ratio, bool signed_bucket) {
        distributor_params_t params = init_distributor_param(slice_length, msm_length, buf_extend_ratio, signed_bucket);
...
// Compute statistics
void compute_stats(const std::vector<double>& data, double& min, double& max, double& avg, double& stddev) {
    if (data.empty()) {
...
int main() {
    ...
    csv << "slice_length,log2_msm_length,msm_length,buf_extend_ratio,signed_bucket,min,max,avg(us),stddev\n";

    std::vector<uint32_t> slice_lengths = {6, 8, 10, 12, 14};
    std::vector<uint32_t> log2_msm_lengths = {10, 12, 14, 16, 18, 20}; // log2(msm_length)
            ...
            for (double buf_extend_ratio : buf_extend_ratios) {
                for (bool signed_bucket : signed_buckets) {
                    if(slice_length >= log2_msm_length) {
                        // std::cerr << "Error: slice_length must be less than log2_msm_length." << std::endl;
                        continue;
                    }
                    std::cout << "Testing slice_length: " << slice_length
                              << ", log2_msm_length: " << log2_msm_length
                              << ", buf_extend_ratio: " << buf_extend_ratio
                              << ", signed_bucket: " << (signed_bucket ? "true" : "false") << std::endl;
                    // Run the test multiple times for statistics
                    std::vector<double> results;
                    for (int trial = 0; trial < trial_num; ++trial) {
                        double perf = run_performance_test(slice_length, msm_length, buf_extend_ratio, signed_bucket);
...
ffi::Error DistributeImpl(uint32_t window_num,
                          uint32_t regular_bucket_num,
                          uint32_t special_bucket_num,
                          uint32_t msm_length,
                          uint32_t fixed_regular_padding_size,
    ...
    Distributor distributor(window_num, regular_bucket_num, special_bucket_num, msm_length);
    distributor.set_fixed_padding_size(fixed_regular_padding_size, fixed_special_padding_size);
...
ffi::Error DistributeBufImpl(uint32_t window_num,
                          uint32_t regular_bucket_num,
                          uint32_t special_bucket_num,
                          uint32_t msm_length,
                          uint32_t fixed_regular_padding_size,
    ...
    distributor.set_points_list(points_list.typed_data());
    distributor.set_zeros(zero.typed_data());
    distributor.set_output_buffers(regular_buckets->typed_data(), special_buckets->typed_data());
...
ffi::Error DistributeBufSignedImpl(uint32_t window_num,
                          uint32_t regular_bucket_num,
                          uint32_t special_bucket_num,
                          uint32_t msm_length,
                          uint32_t fixed_regular_padding_size,
                          ...
                          ffi::Buffer<ffi::S32> slices_list,
                          ffi::Buffer<ffi::U32> points_list,
                          ffi::Buffer<ffi::U32> neg_points_list,
                          ffi::Buffer<ffi::U32> zero,
                          ffi::ResultBuffer<ffi::U32> regular_buckets,
                          ffi::ResultBuffer<ffi::U32> special_buckets,
                          ffi::ResultBuffer<ffi::U32> metadata) {
...
ffi::Error DistributeBufZeroImpl(uint32_t window_num,
                          uint32_t regular_bucket_num,
                          uint32_t special_bucket_num,
                          uint32_t msm_length,
                          uint32_t fixed_regular_padding_size,
