...
import utils
from utils import JaxKernelContextBase, hash_args, jax_jit_lower_compile, load_jax_executable, store_jax_executable
import numpy as np
...
class FusionMSMContext(
  ...
  def _bucketize_regular_windows(
    ...
    inv_rank = zeros_bn.at[batch_idx, sorted_indices].set(within_rank, out_sharding=sharding)
    ...
    output = output.at[inv_rank, :, batch_idx, bucket_ids_i32].set(items_b, out_sharding=sharding_out)
    return output
  ...
  def _bba_and_bws(
    ...
    if self.use_sharding:
      ...
      sharding = jax.sharding.NamedSharding(self.sharding_mesh, P(None, None, self.mesh_axes, None))
