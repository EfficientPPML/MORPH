...
class OldCPUDistributionMSMContext(CPUDistributionMSMContextBase):
  ...
  def multiscalar_multiply(self, tiled_slices: jnp.ndarray):
    ...
    for tile_index in range(self.tile_num):
      ...
      last_window_buckets = jax.device_put(last_window_buckets, jax.devices()[0])
  ...
  def _init_config_parameters(self):
    ...
    if added_padding > 0:
      if added_padding / orig_bucket_num_last_window > 0.1:
        print(f"[bucket_num_last_window] Added {added_padding} to make it divisible by 8, but it is too large for {orig_bucket_num_last_window}. Setting to 0.")
        added_padding = 0
      else:
        print(
