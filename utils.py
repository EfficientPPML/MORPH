...
_serialized_jax_kernel_dir = _config.get("serialized_jax_kernel_dir", "./deployments/")
...
def gen_twiddle_matrix(rows, cols, q, omega):
  ...
  """
  warnings.warn(
      "gen_twiddle_matrix is deprecated. Use jaxite_ntt.number_theory_transform.utils.gen_twiddle_matrix instead.",
      UserWarning,
      stacklevel=2,
  )
...
def gen_twiddle_matrix_inv(rows, cols, q, omega):
  ...
  """
  warnings.warn(
      "gen_twiddle_matrix_inv is deprecated. Use jaxite_ntt.number_theory_transform.utils.gen_twiddle_matrix_inv instead.",
      UserWarning,
      stacklevel=2,
  )
...
def rns_reconstruct(residues, moduli, crt_factors):
  return sum([residues[i] * crt_factors[i] for i in range(len(residues))]) % math.prod(moduli)
...
def modular_matrix_np_u32_to_u8_bat_4d(matrix: np.ndarray, modulus: int):
  ...
  matrix_u64_byteshifted = np.array([matrix_u64 << (8 * byte_idx) for byte_idx in range(4)], dtype=np.uint64)
  # shape is (4, rows, cols)
  matrix_u64_byteshifted = matrix_u64_byteshifted.transpose(1, 0, 2)
  matrix_u64_byteshifted_mod_modulus = (matrix_u64_byteshifted % modulus).astype(np.uint32)
  matrix_u8 = matrix_u64_byteshifted_mod_modulus.view(np.uint8).reshape(rows, 4, cols, 4)
  return matrix_u8
...
def read_external_msm_file(path, type: str):
  if type == "scalars":
    scalars = []
    with open(path, "r", newline="", encoding="utf-8") as csvfile:  # Handle potential encoding issues
...
def slice_scalars(scalars: List, scalar_bits: int, slice_length: int) -> List[List]:
...
def pad_jax_array(array: jnp.ndarray, target_shape: tuple) -> jax.Array:
  if array.shape == target_shape:
    return array
  assert len(array.shape) == len(target_shape), "array and target_shape must have the same number of dimensions"
  pad_width = []
...
class JaxKernelContextBase:
  ...
  def create_named_sharding(self, shape: tuple, axes: list[int]) -> tuple[jax.sharding.NamedSharding, tuple]:
    ...
    if len(axes) == 0:
      ...
      return jax.sharding.NamedSharding(self.sharding_mesh, partition_spec), shape

    assert len(axes) >= 1, "Must specify at least 1 axis"
    assert len(axes) <= 2, "Sharding along >= 3 axes is not supported"
    assert hasattr(self, 'sharding_mesh') and self.sharding_mesh is not None, (
    ...
    if len(axes) == 1:
      ...
      if shape[axis] < total_devices * _MIN_ELEMS_PER_DEVICE:
        spec = [None] * len(shape)
        partition_spec = jax.sharding.PartitionSpec(*spec)
        warnings.warn(f"Shape {shape} is too small for sharding, replicating the axis {axis}", stacklevel=2)
      ...
      if shape[axis] % total_devices != 0:
        warnings.warn(f"Shape {shape} is not divisible by total_devices {total_devices}, padding the axis {axis} to {math.ceil(shape[axis] / total_devices) * total_devices}", stacklevel=2)
        padded_shape[axis] = math.ceil(shape[axis] / total_devices) * total_devices
      ...
      def _compute_candidate(spec_list, shard_plan):
        ...
        mesh_label is either a single mesh axis name or a tuple of all mesh axis names."""
        p = list(shape)
        waste = 0
      ...
      cand_a = _compute_candidate(spec_a, [(mesh_axis_names[0], axis0), (mesh_axis_names[1], axis1)])

      # Candidate B: mesh_x -> axis1, mesh_y -> axis0
      spec_b = [None] * len(shape)
      spec_b[axis1] = mesh_axis_names[0]
      spec_b[axis0] = mesh_axis_names[1]
      cand_b = _compute_candidate(spec_b, [(mesh_axis_names[0], axis1), (mesh_axis_names[1], axis0)])
      ...
      if best_waste > 0:
        warnings.warn(f"Shape {shape} requires padding for sharding, padded to {tuple(padded_shape)}")
...
def store_jax_kernel(func: Callable, *args, **kwargs) -> Callable:
...
def load_jax_kernel(name: str, alternative_callable: Callable = None) -> Callable:
  path = os.path.join(_serialized_jax_kernel_dir, f"{name}.jax")
...
def store_jax_executable(func: Callable, *args, **kwargs) -> Callable:
  name = kwargs.get("name")
  store_dir = os.path.join(_serialized_jax_kernel_dir, f"{name}")
  if not os.path.exists(store_dir):
    os.makedirs(store_dir)
  compiled_kernel = jax_jit_lower_compile(func, *args)
  serialized_executable, in_tree, out_tree = serialize_executable.serialize(compiled_kernel)
  with open(os.path.join(store_dir, f"executable.jax"), "wb") as f:
    f.write(serialized_executable)
  with open(os.path.join(store_dir, f"in_out_trees.pkl"), "wb") as f:
    pickle.dump((in_tree, out_tree), f)
  print(f"stored jax executable to {store_dir}")


def load_jax_executable(name: str) -> Callable:
  store_dir = os.path.join(_serialized_jax_kernel_dir, f"{name}")
  if not os.path.exists(store_dir):
    return None
  print(f"loading jax executable from {store_dir}")
  with open(os.path.join(store_dir, f"executable.jax"), "rb") as f:
    serialized_executable = bytearray(f.read())
  with open(os.path.join(store_dir, f"in_out_trees.pkl"), "rb") as f:
    in_tree, out_tree = pickle.load(f)
  return serialize_executable.deserialize_and_load(serialized_executable, in_tree, out_tree)
...
def _get_hash_length() -> int:
  ...
  config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "configurations.toml")
...
def _serialize(arg: Any) -> str:
  ...
  if isinstance(arg, dict):
    items = ";".join(f"{_serialize(k)}->{_serialize(v)}" for k, v in sorted(arg.items(), key=lambda kv: repr(kv[0])))
